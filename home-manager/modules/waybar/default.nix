...
let 
   ...
   # nouveau
    waybar_config = import ./nouveau/config.nix { inherit osConfig config lib pkgs; };
    waybar_style = import ./nouveau/style.nix { inherit (config) colorscheme; };
in
{
   options.teletypeOne.hm.waybar = {
      enable = mkEnableOption "Enable waybar, not stolen from https://github.com/redyf/nixdots/tree/main";
   };
